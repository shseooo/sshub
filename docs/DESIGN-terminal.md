...
## 14. 즐겨찾기 경로 (2026-09-18)

pane 우클릭 → "즐겨찾기 경로…" → 설정에 저장한 폴더 목록(2단 메뉴). 고르면
`cd <경로>` + Enter를 그 pane의 PTY로 보낸다. 목록은 설정 화면 "Favorite Paths"
카드에서 직접 입력(Enter/추가) 또는 폴더 선택 대화상자로 넣고, 행의 🗑로 지운다.

- **코어** `sshub_core::favorite_paths` — `normalize_path`(trim, 제어 문자
  거부) · `add_favorite`(중복 제거) · `cd_command`(인용). 저장은
  `Settings::favorite_paths: Vec<String>`(camelCase `favoritePaths`, 비면 키
  생략). `Settings::normalize`가 로드 시 같은 규칙으로 걷어낸다.
- **인용 규칙**: 앞의 `~` / `~/`만 셸 확장을 위해 인용 밖에 두고 나머지는 전부
  POSIX 작은따옴표(`'` → `'\''`). 공백·`$`·백틱·`;`는 글자 그대로 경로가 된다.
  `~user/…` 형태는 확장하지 않는다(글자 그대로). 개행·제어 문자는 어떤 인용으로도
  한 줄에 못 넣으므로 값 자체를 거부한다.
- **원격 pane에서도 활성**: `~/work`처럼 양쪽에 있는 폴더를 즐겨찾기로 두는
  쓰임이 있고, 존재하지 않으면 셸이 에러를 낸다 — 앱이 미리 검증하지 않는다.
  같은 이유로 로컬 존재 여부도 검사하지 않는다.
- **UI**: `menus.rs::open_favorite_paths_menu`가 `run_in_pane`(§13과 공유)을
  쓴다. 빈 목록이면 "설정에서 추가" 비활성 항목 하나를 보인다 — 메뉴가 통째로
  안 뜨면 고장으로 보인다.
- 한계: §13과 같이 pane의 포그라운드 프로그램이 셸이라고 가정한다.
